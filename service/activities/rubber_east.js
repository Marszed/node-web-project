...
//--------------------------- 日志配置 ---------------------------//
var li = uutil.getLogger('INFO', 'service/activities/rubber_east.js');
var ld = uutil.getLogger('DEBUG', 'servie/activities/rubber_east.js');
...
var middleware_check_fake_wx_id = require( '../../lib/middleware_check_fake_wx_id' );
...
var middleware_check_only_once = function (req, res, next) {
    ...
    dashixiong.getUserActivity( user.id, 5, function (err, ret) {//5 为东区抢货活动
        var userAc = ret[0];
...
var middleware_right_time = function (req, res, next) {
    var user = req.user;
    var start_d = Date.UTC(2014, 3, 21, 22, 30);//月份是从0开始数的, UTC时间
    ...
    console.log( 'now', now );
    console.log( 'start_d', start_d);
    console.log( 'deta', now - start_d);
    console.log(  now - start_d >  1000*60*10 );

    if( (now - start_d) > 1000*60*10 ) {//活动时间只有10分钟
        render(req, res, 'wx/activity/msg', {
            msg : '你刚才干嘛去啦！？来晚啦! 活动已经结束咯!' 
        });
        return;
    }
...
var middleware_check_num_in_warehouse = function (req, res, next){
    ...
    dashixiong.getProductsNumsInWareshouse( w_id - 0, ids, function (err, ret) {
        ...
        if( ret_products.length > 5 ){//最多只能抢5样
            ret_products.length = 5; 
        }
...
//活动首页
app.get('/dashixiongwx/activity/5', 
        middleware_check_fake_wx_id, //防止虚假微信id
...
function(req, res){
    //render( req, res, 'wx/activity/msg', {
    //    msg : '小卖部已经被瞬间抢光光！明天给大家送货~ 你们这帮强盗哇！'
    //});
    //return;
    ...
    console.log( '=============== 开抢 =============== ' );
    li.info( '============== 广东工程东区搞活动 ============ shopId=='+req.user.shopId );
    li.info( req.user );
    ...
    dashixiong.getSectionsWithProducts(req.user.shopId, req.user.id, function(err, sections){
        render( req, res, 'wx/activity/ac_5/index', {
            layout : false,
...
app.post('/dashixiongwx/activity/addr', 
...
function(req, res){
	...
	var order = {
        ...
        extra : 'activity_5'//有这个字段的订单都将认为是活动订单, 而不被计入正常订单里边
	};
...
app.post('/dashixiongwx/activity/order/finish', 
    middleware_load_user_by_wx_id,//用户信息
    middleware_get_config_of_shop,//店铺配置
    middleware_check_only_once,//只能参加一次活动
    middleware_load_products_map, //获取产品信息
    middleware_check_num_in_warehouse, //获取产品库存
function(req, res){
	...
	var response = function(){
		render(req, res, 'wx/activity/ac_5/order_sus', {
			layout : true,
			user_id : user_id,
			order_id : order_id,
			wx_id : wx_id,
